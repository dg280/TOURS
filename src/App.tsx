    ...
    const fetchData = async () => {
      try {
        ...
        const { data: configData, error: configError } = await supabase
          ...
          .maybeSingle();
        if (configError)
          console.error("Error fetching site_config:", configError);
        ...
        if (profileData && profileData.value) {
          ...
          const dbBio =
            lang === "en"
              ...
              : lang === "es"
                ? profile.bio_es
                : profile.bio || profile.bio1;
          if (dbBio) setGuideBio(dbBio);
          else if (profile.bio1)
            setGuideBio(
              profile.bio1 + (profile.bio2 ? "\n\n" + profile.bio2 : ""),
            );

          if (
            profile.different_photos &&
            Array.isArray(profile.different_photos) &&
            profile.different_photos.length === 3
          ) {
            setDifferentPhotos(
              profile.different_photos as [string, string, string],
            );
          }
        }
      } catch (err) {
