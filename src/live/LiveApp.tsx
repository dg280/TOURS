...
export default function LiveApp() {
    ...
    const sessionCode = window.location.pathname.split('/').pop() || new URLSearchParams(window.location.search).get('code');
    ...
    useEffect(() => {
        ...
        const fetchSession = async () => {
            ...
            try {
                ...
                const { data: ech } = await supabase.from('live_echoes').select('*').eq('session_id', sess.id).order('created_at', { ascending: true });
                setEchoes(ech || []);
    ...
    if (!session) {
        return (
            <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 text-center">
                ...
                <h1 className="text-2xl font-bold text-white mb-2">Session introuvable</h1>
                <p className="text-gray-400 mb-6">Le code de session est invalide ou la visite est terminée.</p>
                <Button className="bg-amber-600" onClick={() => window.location.href = '/'}>Retour à l'accueil</Button>
            </div>
        );
    }
            ...
            <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 bg-gradient-to-b from-[#111] to-[#0a0a0a]">
                <div className="w-full max-w-sm space-y-8 animate-in fade-in zoom-in duration-500">
                    <div className="text-center">
                        <h1 className="text-4xl font-black text-white italic tracking-tighter mb-2">TOURS<span className="text-amber-500">&</span>DETOURS</h1>
                        <p className="text-amber-500 font-bold uppercase tracking-widest text-xs">Expérience Live</p>
                    </div>
                    ...
                    <Card className="bg-[#1a1a1a] border-[#333] shadow-2xl">
                        <CardContent className="p-8 space-y-6">
                            <div className="text-center space-y-2">
                                ...
                                <h2 className="text-xl font-bold text-white">Prêt pour l'aventure ?</h2>
                                <p className="text-sm text-gray-500">Entrez votre nom pour rejoindre le groupe.</p>
                            </div>
                            <form onSubmit={handleNameSubmit} className="space-y-4">
                                <Input
                                    ...
                                    autoFocus
                                />
                                <Button type="submit" className="w-full bg-amber-600 hover:bg-amber-700 h-12 text-lg font-bold shadow-lg shadow-amber-600/20">
                                    C'est parti !
                                </Button>
                            </form>
                        </CardContent>
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-gray-100 flex flex-col selection:bg-amber-500/30">
            ...
            <header className="sticky top-0 z-50 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-amber-500/20 p-4">
                <div className="max-w-md mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-amber-600 rounded-lg flex items-center justify-center font-black italic text-sm">T&D</div>
                        <div>
                            <h1 className="text-sm font-bold leading-tight">{tour?.title || 'Chargement...'}</h1>
                            <p className="text-[10px] text-amber-500 font-bold uppercase">Guide : Antoine</p>
                        </div>
                    </div>
                    <Badge variant="outline" className="text-amber-500 border-amber-500/30 text-[10px]">LIVE</Badge>
                </div>
            </header>
            ...
            <main className="flex-1 max-w-md mx-auto w-full p-4 space-y-6">
                ...
                {session.urgent_message && (
                    <div className="bg-red-600 text-white p-4 rounded-2xl flex items-start gap-3 shadow-xl shadow-red-600/20 animate-bounce-subtle">
                        ...
                        <X className="w-4 h-4 opacity-50 cursor-pointer" onClick={() => setSession({ ...session, urgent_message: null })} />
                    </div>
                )}
                    ...
                    {!isLastStop ? (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-xs font-bold text-amber-500 uppercase tracking-widest">Étape en cours</span>
                            </div>
                            <Card className="bg-[#1a1a1a] border-amber-500/30 overflow-hidden shadow-2xl relative">
                                ...
                                <div className="p-6 pt-10 space-y-4">
                                    ...
                                    <p className="text-gray-400 text-sm leading-relaxed">
                                        {currentStop?.description || "Suivez le guide pour découvrir la suite !"}
                                    </p>
                                    {currentStop?.image && (
                                        <div className="rounded-xl overflow-hidden border border-white/5 shadow-inner">
                                            <img src={currentStop.image} alt={currentStop.name} className="w-full grayscale hover:grayscale-0 transition-all duration-700" />
                                        </div>
                                    )}
                                </div>
                            </Card>
                        ...
                        <Card className="bg-gradient-to-br from-amber-600 to-amber-800 border-none text-white p-8 text-center space-y-6 shadow-2xl shadow-amber-600/20">
                            ...
                            <div className="space-y-2">
                                <h2 className="text-3xl font-black italic tracking-tighter">Merci pour cette visite !</h2>
                                <p className="text-amber-100 opacity-90">J'espère que vous avez passé un moment inoubliable.</p>
                            </div>
                            ...
                            <p className="text-[10px] uppercase font-bold tracking-widest opacity-60">Fait avec passion par Tours & Detours</p>
                        </Card>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
                        ...
                        {media.map((item) => (
                            <div key={item.id} className="relative w-40 h-40 flex-shrink-0 group">
                                <img src={item.url} alt="Shared" className="w-full h-full object-cover rounded-2xl border border-white/5 shadow-lg group-hover:scale-105 transition-all duration-500" />
                                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent rounded-b-2xl">
                                    <p className="text-[10px] font-bold text-white truncate">{item.uploaded_by}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <Card className="bg-[#111] border-[#222] h-64 flex flex-col overflow-hidden rounded-2xl shadow-inner">
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 scroll-smooth">
                            {echoes.length === 0 && (
                                <p className="text-center text-gray-600 italic text-xs pt-10">Dites quelque chose au groupe !</p>
                            )}
                            {echoes.map((echo) => (
                                <div key={echo.id} className={`flex flex-col ${echo.user_name === userName ? 'items-end' : 'items-start'}`}>
                                    <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${echo.user_name === userName
                                        ? 'bg-amber-600 text-white rounded-tr-none'
                                        : 'bg-[#1a1a1a] text-gray-200 border border-[#222] rounded-tl-none'
                                        }`}>
                                        <p className="text-[10px] font-black italic opacity-50 mb-1">{echo.user_name}</p>
                                        <p className="leading-snug">{echo.message}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <form onSubmit={sendEcho} className="p-3 bg-[#0a0a0a] border-t border-[#222] flex gap-2">
                            <Input
                            ...
                            />
                            <Button type="submit" size="icon" className="bg-amber-600 h-10 w-10 shrink-0">
            ...
            <footer className="p-8 text-center border-t border-[#111] bg-[#0a0a0a]">
                <div className="max-w-md mx-auto space-y-4">
                    <h2 className="text-2xl font-black text-[#222] italic tracking-tighter">TOURS<span className="text-amber-900/10">&</span>DETOURS</h2>
                    <div className="flex justify-center gap-4 text-gray-600">
                        <ExternalLink className="w-4 h-4" />
                        <span className="text-[10px] font-bold uppercase tracking-widest">www.toursandetours.com</span>
                    </div>
                </div>
            </footer>
        </div>
