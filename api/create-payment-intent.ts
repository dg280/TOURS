...
function getClients() {
  if (!stripe) {
    const secretKey =
      process.env.STRIPE_SECRET_KEY || process.env.test_stripe_pv;
    if (!secretKey) {
      console.error(
        "CRITICAL: Stripe Secret Key is missing (checked STRIPE_SECRET_KEY and test_stripe_pv)",
      );
      throw new Error("Stripe is not configured");
    }
    console.log(
      `[Stripe] Initialized using ${secretKey.startsWith("sk_test_") ? "TEST" : "LIVE"} key.`,
    );
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (!process.env.STRIPE_SECRET_KEY && !process.env.test_stripe_pv) {
      ...
      return res
        .status(500)
        .json({ error: "Configuration Stripe manquante (Secret Key)" });
    }
    if (
      !process.env.VITE_SUPABASE_URL ||
      !process.env.SUPABASE_SERVICE_ROLE_KEY
    ...
    if (!tourId || !participants) {
      return res
        .status(400)
        .json({ error: "Données manquantes (tourId ou participants)" });
    }
    ...
    if (isNaN(participantsInt) || participantsInt < 1 || participantsInt > 50) {
      return res
        .status(400)
        .json({ error: "Nombre de participants invalide (1–50)" });
    }
    ...
    if (fetchError || !tour) {
      ...
      return res
        .status(404)
        .json({ error: "Tour non trouvé en base de données" });
    }
    ...
    const totalAmount = (baseAmount + 0.3) / 0.956;
    ...
    const clientIp = Array.isArray(fwdHeader)
      ? fwdHeader[0]
      : fwdHeader || "unknown";
