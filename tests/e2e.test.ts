...
test.describe("Full Site Verification - Tours & Detours", () => {
  ...
  test("Data: Multi-language Support & SEO", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText(
      /Unique private Tours and Experiences in Barcelona and Catalonia/i,
    );
  ...
  test("UI: Tour Dialog Logic & Overflow Check", async ({ page }) => {
    ...
    expect(
      isOverflowing,
      "The 'Book Now' button text is overflowing its container!",
    ).toBe(false);
  });
  ...
  test("Performance: Assets 404 check", async ({ page }) => {
    ...
    expect(
      brokenAssets,
      `Found ${brokenAssets.length} broken assets: ${brokenAssets.join(", ")}`,
    ).toHaveLength(0);
  });
  ...
  test("UI: About Page Sticky & Mobile Responsiveness", async ({ page }) => {
    ...
    expect(
      hasHorizontalScroll,
      "Horizontal scroll detected on mobile! Overflow issue.",
    ).toBe(false);
  });

  test("Admin: Smoke Test & Login Access", async ({ page }) => {
    await page.goto("/admin.html");
    await expect(page.locator("h2, h1")).toContainText(
      /Admin|Connexion|Connecter|Tours.*Detours/i,
    );
  ...
  test("Conversion: Booking Funnel Walkthrough", async ({ page }) => {
    ...
    await expect(page.locator("text=Step 2 of 5").first()).toBeVisible({
      timeout: 10000,
    });
  ...
  test("Regression: Booking Funnel in EN — calendar + step navigation", async ({
    page,
  }) => {
  ...
  test("Regression: Booking Funnel in ES — calendar + step navigation", async ({
    page,
  }) => {
    ...
    await nextBtn.scrollIntoViewIfNeeded();
    await nextBtn.click({ force: true });
    await expect(page.locator("text=/Paso 2 de 5/").first()).toBeVisible({
      timeout: 10000,
    });
  ...
  test("Security: Outbound Links Safety", async ({ page }) => {
    ...
    for (const link of externalLinks) {
      ...
      expect(
        rel,
        `Link ${await link.getAttribute("href")} missing noopener noreferrer`,
      ).toMatch(/noopener.*noreferrer|noreferrer.*noopener/);
    }
  });
  ...
  test("UI: Bon à savoir tab loads correctly", async ({ page }) => {
    ...
    await page
      .locator('div[role="dialog"] [role="tab"]')
      .filter({ hasText: /Savoir|Know|Útil/i })
      .first()
      .click();
    ...
    await expect(
      page.locator(
        'div[role="dialog"] div[role="tabpanel"][data-state="active"]',
      ),
    ).toBeVisible();
  });
  ...
  test("Features: Category Filtering", async ({ page }) => {
    ...
    const cultureBtn = categorySection.getByRole("button", {
      name: /Culture/i,
    });
    if (await cultureBtn.isVisible()) {
      ...
      await expect(
        categorySection.locator(".embla__slide").first(),
      ).toBeVisible();
    }
  });
  ...
  test("Features: Live Session Smoke Test", async ({ page }) => {
    ...
    if (await liveBtn.isVisible()) {
      await liveBtn.click();
      await expect(
        page.locator("text=/Rejoindre|Join|Unirme/i").first(),
      ).toBeVisible();
    }
    ...
    await expect(
      page
        .locator("text=/aventure|ready|aventura|introuvable|found|encontrada/i")
        .first(),
    ).toBeVisible();
  });
  ...
  test("Admin: Tour Management Flow", async ({ page }) => {
    ...
    test.skip(
      ...
      "Admin localStorage bypass disabled in production builds",
    );
    await page.goto("/admin.html");
    // Bypass login
    ...
    const newTourBtn = page.getByRole("button", {
      name: /Nouveau Tour|New Tour/i,
    });
    if (await newTourBtn.isVisible()) {
      await newTourBtn.click();
      await expect(
        page.locator("text=/Édition Tour|Edit Tour|Tour Details/i"),
      ).toBeVisible();
      await page
  ...
  test("UI: Tour Content Translation", async ({ page }) => {
    ...
    const durationText = page.locator(
      'div[role="dialog"] span:has-text("Day")',
    );
    ...
    const durationCard = page
      .locator("#top-tours, section#tours")
      .locator('span:has-text("Día")')
      .first();
  ...
  test("UI: Contact Form — Fill & Submit", async ({ page }) => {
    ...
    await expect(page.locator("#message")).toHaveValue(
      "Message de test automatisé E2E.",
    );
  ...
  test("Conversion: Booking Funnel Step 2 — Full Form Fill", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Accepter|Accept/i })
    ...
    if (await addressField.isVisible())
      await addressField.fill("123 Rue de Test");

    await expect(page.locator("input#booking-name")).toHaveValue(
      "Test E2E User",
    );
    await expect(page.locator("input#booking-email")).toHaveValue(
      "test@e2e.com",
    );
    ...
    const atStep3 = await page
      .locator("text=/Step 3 of 5/")
      .isVisible()
      .catch(() => false);
    const atStep2 = await page
      .locator("text=Step 2 of 5")
      .isVisible()
      .catch(() => false);
  ...
  test("Admin: Dashboard Metrics Render", async ({ page }) => {
    ...
    await expect(page.locator("text=/En attente/i").first()).toBeVisible({
      timeout: 15000,
    });
  ...
  test("Admin: Reservations Tab — Search & Status Filter", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /Réservations/i })
      .first()
      .click();
    await page.waitForTimeout(500);
  ...
  test("Admin: Avis Clients Tab Loads", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /Avis clients/i })
      .first()
      .click();
    await page.waitForTimeout(500);

    await expect(page.locator("text=/Avis|avis|Review/i").first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Admin: Suivi (Live Sessions) Tab Loads", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/Suivi Opérationnel|Pick-ups|Planning/i").first(),
  ...
  test("Admin: Mon Profil — Photo Upload Slots Visible", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /Mon Profil|My Profile/i })
      .first()
      .click();
    await page.waitForTimeout(500);
    ...
    await expect(
      page.locator("text=/différent|Différent/i").first(),
    ).toBeVisible();

    // At least 1 photo change button visible
    const changeButtons = page
      .locator("button")
      .filter({ hasText: /Changer|Photo|Modifier/i });
  ...
  test("UI: Footer Nav Links & Legal Modals", async ({ page }) => {
    ...
    await expect(
      footer.getByRole("link", { name: /Nos Tours|Tours/i }).first(),
    ).toBeVisible();
    await expect(
      footer.getByRole("link", { name: /Guide|Votre Guide/i }).first(),
    ).toBeVisible();
    await expect(
      footer.getByRole("link", { name: /Avis|Reviews/i }).first(),
    ).toBeVisible();
    await expect(
      footer.getByRole("link", { name: /Contact/i }).first(),
    ).toBeVisible();
    ...
    // Legal notice modal
    await footer
      .getByRole("button", { name: /Mentions légales|Legal Notice/i })
      .click();
    await expect(page.locator('div[role="dialog"]')).toBeVisible({
      timeout: 5000,
    });
    await expect(page.locator('div[role="dialog"]')).toContainText(
      /Mentions légales|Legal Notice|Éditeur|Hébergement/i,
    );
    await page.keyboard.press("Escape");
    await page.waitForTimeout(300);
    ...
    // Privacy modal
    await footer
      .getByRole("button", { name: /Confidentialité|Privacy/i })
      .click();
    await expect(page.locator('div[role="dialog"]')).toBeVisible({
      timeout: 5000,
    });
    await expect(page.locator('div[role="dialog"]')).toContainText(
      /Confidentialité|Privacy|RGPD|données/i,
    );
    await page.keyboard.press("Escape");
  });
});
